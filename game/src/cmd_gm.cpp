...
#include "../../common/tables.h" // TFakePlayerLoginData gibi yapýlarý tanýmlayan baþlýk dosyasýný ekle.
...
ACMD(do_fake_player)
{
    ...
    if (!*arg1)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "usage: fake_player login");
        ...
        ch->ChatPacket(CHAT_TYPE_INFO, "usage: fake_player reload");
        return;
    }

    if (0 == strcmp(arg1, "reload"))
    {
        // Aktif botlarýn verilerini geçici olarak kaydet
        std::vector<TFakePlayerLoginData> activeBotsData;
        CFakePlayerManager::Instance().SaveActiveFakePlayersData(activeBotsData);

        // Tüm botlarý çýkýþ yaptýr
        std::vector<std::string> activeNames;
        CFakePlayerManager::Instance().GetActiveFakePlayerNames(activeNames);
        for (const auto& name : activeNames)
        {
            CFakePlayerManager::Instance().FakePlayerLogout(name.c_str(), ch);
        }

        // Konfigürasyon dosyalarýný yeniden yükle
        TFakePlayerReloadResult result = CFakePlayerManager::Instance().ReloadConfigurations();
        
        if (result.success)
        {
            // Aktif botlarý tekrar giriþ yaptýr
            for (const auto& botData : activeBotsData)
            {
                CFakePlayerManager::Instance().FakePlayerLogin(ch, &botData);
            }
            ch->ChatPacket(CHAT_TYPE_INFO, "Bot configurations successfully reloaded.");
            ch->ChatPacket(CHAT_TYPE_INFO, "%d bots were restored.", result.reloadedBotCount);
        }
        else
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Failed to reload bot configurations: %s", 
    ...
    if (0 == strcmp(arg1, "login"))
    {
        ...
        std::string randomName = CFakePlayerManager::Instance().GetRandomName();
        if (randomName.empty())
        {
        ...
        if (CHARACTER_MANAGER::instance().FindPC(randomName.c_str()))
        {
            CFakePlayerManager::Instance().AddNameBackToList(randomName); // Ýsmi geri ekle
            ch->ChatPacket(CHAT_TYPE_INFO, "This name is already in use by a real player.");
            return;
        }

        TFakePlayerLoginData fakePlayerLoginData;
        strlcpy(fakePlayerLoginData.szName, randomName.c_str(), sizeof(fakePlayerLoginData.szName));
    ...
    else if (0 == strcmp(arg1, "logout"))
    {
        if (!*arg2)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Please specify a name to logout.");
            return;
        }
    ...
    else if (0 == strcmp(arg1, "fullon"))
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            if (randomName.empty())
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "No more names available in the name list. Spawned %d bots.", spawnedCount);
                return;
            }
            ...
            if (CHARACTER_MANAGER::instance().FindPC(randomName.c_str()))
            {
                CFakePlayerManager::Instance().AddNameBackToList(randomName);
                continue;
            }
            ...
            ch->ChatPacket(CHAT_TYPE_INFO, "Fake player %s logged in.", randomName.c_str());
            spawnedCount++;
        }
        
        if (spawnedCount < count)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Only %d bots could be spawned.", spawnedCount);
        }
    }
    else if (0 == strcmp(arg1, "fulloff"))
    {
        ...
        // Tüm aktif botlarýn listesini al
        std::vector<std::string> activeNames;
        CFakePlayerManager::Instance().GetActiveFakePlayerNames(activeNames);

        int logoutCount = std::min(count, static_cast<int>(activeNames.size()));
        for (int i = 0; i < logoutCount; ++i)
        {
            CFakePlayerManager::Instance().FakePlayerLogout(activeNames[i].c_str(), ch);
        }

        ch->ChatPacket(CHAT_TYPE_INFO, "%d bots have been logged out.", logoutCount);
    }
}
#endif
