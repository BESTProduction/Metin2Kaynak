...
// Ölüm sonrasý yeniden doðma eventi
EVENTFUNC(fake_player_dead_event)
{
    char_event_info* info = dynamic_cast<char_event_info*>(event->info);
    if (info == NULL)
    {
        sys_err("fake_player_dead_event> <Factor> Null pointer");
        return 0;
    }
    ...
    // Karakteri diriltelim
    ch->Show(ch->GetMapIndex(), ch->GetX(), ch->GetY(), 0); // Mevcut konumda dirilme
	/*ch->Show(ch->GetMapIndex(), spawn_x, spawn_y, 0);*/ 	// Veya baþka bir konumda dirilmek için
