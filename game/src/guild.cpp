...
#ifdef ENABLE_FAKE_PLAYER_SYSTEM
	// Fake oyuncu kontrolü ekleniyor
	if (ch->IsFakePlayer())
	{
		// 5 saniye sonra "lonca daveti reddedildi" mesajını göndermek için bir zamanlayıcı oluşturuyoruz
		event_create([](LPCHARACTER ch) {
			if (ch)
				ch->ChatPacket(CHAT_TYPE_INFO, "Oyuncu lonca davetini reddetti.");
			return 0;
		}, PASSES_PER_SEC(5), ch);
