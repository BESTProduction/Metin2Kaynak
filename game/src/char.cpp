...
/*#ifdef __FAKE_PLAYER__
			memcpy((void*)&addPacket.bLanguage, m_stFakePlayerLanguage.c_str(), sizeof(addPacket.bLanguage));
#endif*/
...
#ifdef __FAKE_PLAYER__
			else if (IsFakePlayer())  // Bot oyuncular için kontrol
			{
				addPacket.dwGuildID = m_dwFakePlayerGuildID;  // Botun lonca ID'sini ayarla
				addPacket.dwNewIsGuildName = 0;  // Botlar için lonca liderliği yok
			}
#endif
			else
			{
...
{
    if (IsRiding())  // Eğer karakter biniyorsa
    {
        StopRiding();  // Binme durumunu durdur
        UpdatePacket();  // Karakter bilgilerini güncelle
    }
}
#endif
