...
void CHorseRider::EnterHorse()
{
    ...
    // CHorseRider, CHARACTER sýnýfýnýn içinde olduðundan
    // this pointer'ýný CHARACTER* olarak cast ediyoruz
    ...
    if (ch->IsFakePlayer())
    {
        ch->HorseSummon(true, false);
        sys_err("IsFakePlayer HorseSummon");
        return;
    }
#endif
    if (GetHorseLevel() <= 0)
