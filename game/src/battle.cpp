...
bool battle_is_attackable(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	/*#ifdef __FAKE_PLAYER__
	// Fake oyuncu kontrolü
	if (ch->IsFakePlayer())
	{
		// Ölü hedeflere saldırmayı engelle
		if (!victim || victim->IsDead())
			return false;
			
		// Aynı krallıktan ise saldırmayı engelle 
		if (victim->GetEmpire() == ch->GetEmpire())
			return false;

		// Farklı krallıktan oyunculara, moblara ve metin taşlarına saldır
		if (victim->GetEmpire() != ch->GetEmpire() || 
			victim->IsMonster() || 
			victim->IsStone())
			return true;
	...
	// Normal oyuncuların fake oyunculara saldırma kontrolü  
	if (victim->IsFakePlayer())
	{
		// Farklı krallıktan saldırabilir
		if (ch->IsPC() && ch->GetEmpire() != victim->GetEmpire())
			return true;
