...
public:
	...
	/*void SetCharType(ECharType type) { m_bCharType = type; }*/
...
protected:
    ...
    public:
        ...
        void SetLanguage(const std::string& stLanguage) { m_stFakePlayerLanguage = stLanguage; }
        const std::string& GetLanguage() const { return m_stFakePlayerLanguage; }

        void SetFakePlayerGuildID(uint32_t dwID) { m_dwFakePlayerGuildID = dwID; }
        uint32_t GetFakePlayerGuildID() const { return m_dwFakePlayerGuildID; }

        void SetFakePlayerBlockEQ(uint8_t bBlock) { m_bFakePlayerBlockEQ = bBlock; }
        uint8_t GetFakePlayerBlockEQ() const { return m_bFakePlayerBlockEQ; }

        void FakePlayerCreatePetSystem();  // Botlar için evcil hayvan sistemi kurar
        void Unmount();  // Karakterin binekten inmesini sağlar

    protected:
        bool m_bIsFakePlayer;  // Karakterin bot olup olmadığını belirten bayrak
        std::string m_stFakePlayerLanguage;  // Botun dili
        uint32_t m_dwFakePlayerGuildID;  // Botun lonca kimliği
        uint8_t m_bFakePlayerBlockEQ;  // Botun ekipman durumu için bloklama bayrağı

        LPEVENT m_pFakePlayerDeadEvent;  // Bot için ölü olayı işaretçisi
#endif
