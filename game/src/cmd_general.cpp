...
ACMD(do_pvp)
{
	...
	// Oyuncu arenadaysa veya arena haritasýnda ise düello isteði engellenir
	...
	// Eðer hedef NPC ise düello isteði engellenir
	if (pkVictim->IsNPC())
		return;

	// Eðer hedef arenadaysa düello isteði engellenir
...
#ifdef ENABLE_FAKE_PLAYER
	// Sahte oyunculara düello isteði gönderilmesini engelle
	if (pkVictim->IsFakePlayer())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Oyuncu düello isteðini engelledi.");
		return;
	}
#endif

#ifdef ENABLE_BATTLE_FIELD
	// Eðer oyuncu savaþ alanýnda ise düello isteði engellenir
	...
	// Eðer oyuncu özel maðaza haritasýnda ise düello isteði engellenir
	...
	// Mesaj engelleme durumu kontrolü
	...
	// Hedef zaten arenadaysa düello isteði engellenir
	if (pkVictim->GetArena() != nullptr)
	{
		pkVictim->ChatPacket(CHAT_TYPE_INFO, "[LS;522]");
		return;
	...
	// Eðer tüm koþullar saðlanýyorsa düello isteði baþlatýlýr
...
ACMD(do_view_equip)
{
	...
	// Eðer sadece GM'ler ekipman görebiliyorsa ve oyuncu GM deðilse iþleme devam etme.
	...
	if (*arg1)
	{
		...
		// Hedef karakter yoksa iþleme devam etme
		if (!tch)
			return;

		// Hedef karakter bir PC (gerçek oyuncu) deðilse ve sahte oyuncu deðilse iþleme devam etme
		if (!tch->IsPC()
#ifdef __FAKE_PLAYER__
			&& !tch->IsFakePlayer()
#endif
		...
		// Sahte bir oyuncunun ekipmanýný görüntülemeyi engelle
#ifdef __FAKE_PLAYER__
		if (tch->IsFakePlayer())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Oyuncu ekipman görüntülemesini devre dýþý býraktý."));
			return;
		}
#endif

#ifdef ENABLE_MESSENGER_BLOCK
		// Engellenmiþ oyunculara eriþimi kýsýtla
		if (MessengerManager::Instance().IsBlocked(ch->GetName(), tch->GetName()))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Engellemeyi kaldýrmak için %s"), tch->GetName());
			return;
		}
		else if (MessengerManager::Instance().IsBlocked(tch->GetName(), ch->GetName()))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s sizi engelledi."), tch->GetName());
			return;
		}
#endif

#ifdef __UNIMPLEMENTED__
		// Oyuncunun ekipman görüntülemeyi engelleme durumu kontrol edilir
		if ((tch->IsBlockMode(BLOCK_VIEW_EQUIP) && !ch->IsGM()) 
			|| (!test_server && tch->IsGM()) ) {
			ch->ChatInfoTrans(("Oyuncu %s ekipmanýný görüntülemeyi engelledi!"), tch->GetName());
			return;
		}
#endif

		// Hedef karakterin ekipman bilgisini gönder
...
ACMD(do_party_request)
{
	...
	{
#ifdef __FAKE_PLAYER__
		// Eðer hedef karakter sahte bir oyuncu ise, parti isteðini engelle
		if (tch->IsFakePlayer())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Oyuncu parti davetini engelledi!");
			return;
		}
#endif

		// Parti isteðini gönder
...
static const char* FN_point_string(int apply_number)
{
	switch (apply_number)
	{
		...
		case POINT_HIT_PCT:	return LC_TEXT("µ¶°ø°Ý %d");
#endif
		...
		case POINT_ACCEDRAIN_RATE:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_RESIST_MAGIC_REDUCTION:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ATTBONUS_CZ:		return LC_TEXT("¾Ç¸¶·ù Å¸°ÝÄ¡ +%d%%");
		case POINT_ATTBONUS_DESERT:	return LC_TEXT("¾Ç¸¶·ù Å¸°ÝÄ¡ +%d%%");
		case POINT_ATTBONUS_INSECT:	return LC_TEXT("¾Ç¸¶·ù Å¸°ÝÄ¡ +%d%%");
		case POINT_ENCHANT_ELECT:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ENCHANT_ICE:		return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ENCHANT_DARK:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ENCHANT_FIRE:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ENCHANT_WIND:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		case POINT_ENCHANT_EARTH:	return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
		...
		case POINT_RESIST_HUMAN: return LC_TEXT("¸¶¹ý ÀúÇ× %d%%");
...
ACMD(do_shop_open)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°³ÀÎ »óÁ¡À» ¿­ ¼ö ¾ø´Â Áö¿ªÀÔ´Ï´Ù"));
		return;
	}
