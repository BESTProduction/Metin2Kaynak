...
{
	// Sahte oyuncu veya gerçek oyuncu kontrolü
	LPCHARACTER tch = CHARACTER_MANAGER::Instance().FindPC(companion.c_str());

#ifdef __FAKE_PLAYER__
	if (tch && tch->IsFakePlayer())
	{
		...
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s baþarýyla engellendi.."), companion.c_str());
		}
		return;
	}
#endif
...
void MessengerManager::__RemoveFromBlockList(MessengerManager::keyB account, MessengerManager::keyB companion)
{
	...
	if (tch && tch->IsFakePlayer())
	{
		...
		{
			it->second.erase(companion);
			m_InverseBlockRelation[companion].erase(account);

			LPCHARACTER ch = CHARACTER_MANAGER::Instance().FindPC(account.c_str());
			if (ch)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s kaldýrýldý."), companion.c_str());
			}
		}
		return;
	}
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s engeli kaldýrýldý."), companion.c_str());
}
